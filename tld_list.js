// tld_list.js — Official IANA TLD List
// Source: https://data.iana.org/TLD/tlds-alpha-by-domain.txt
// Version: 2026042300 (Thu Apr 24 2026) — 1437 TLDs
//
// HOW TO USE in check_link.js:
//   Replace your TLD regex check with: VALID_TLDS.has(tld.toLowerCase())
//
// Include this file BEFORE check_link.js in your HTML:
//   <script src="tld_list.js"></script>
//   <script src="check_link.js"></script>

// The Set gives O(1) lookups — fast even with 1400+ entries.
...
/**
 * Returns true if the given TLD string is in the IANA list.
 * Handles both ASCII TLDs (e.g. 'to', 'com') and
 * punycode internationalised TLDs (e.g. 'xn--p1ai').
 *
...
function isValidTLD(tld) {
  if (!tld) return false;
  return VALID_TLDS.has(tld.replace(/^\./, '').toLowerCase());
}

/**
 * Tries to fetch a fresh copy of the IANA TLD list and update VALID_TLDS.
 * Call this once on page load (optional — the hardcoded list is already current).
 * If the fetch fails for any reason, the hardcoded list is kept as fallback.
 */
async function refreshTLDsFromIANA() {
  try {
    ...
    if (fresh.length > 100) {  // sanity check
      VALID_TLDS.clear();
      fresh.forEach(t => VALID_TLDS.add(t));
      console.log(`[WebSafe] TLD list refreshed from IANA: ${fresh.length} TLDs loaded`);
    }
  } catch (e) {
    console.warn('[WebSafe] Could not refresh TLD list from IANA — using built-in list:', e.message);
  }
}
